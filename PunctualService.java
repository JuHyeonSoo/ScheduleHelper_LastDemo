...
public class PunctualService extends Service implements GeoTask.Geo {
    ...
    //Noti 진동 type 설정
    private final static long[] vibrate = new long[] { 500, 300, 500, 700, 500, 1000 };
    ...
    public void onCreate() {
        ...
        //오늘꺼에 1시간 반을 더할 꺼야
        int which_year = judge_cal.get(Calendar.YEAR); //4자리
        int which_month = judge_cal.get(Calendar.MONTH)+1; //1,2자리
        int which_date = judge_cal.get(Calendar.DATE); //1,2자리
        int which_hour = judge_cal.get(Calendar.HOUR_OF_DAY); //1,2자리
        int which_minute = judge_cal.get(Calendar.MINUTE); //1,2자리
        int Max_date_of_month = judge_cal.getActualMaximum(Calendar.DAY_OF_MONTH); //달의 마지막 날짜
        Log.d("이 달은 몇일 까지 있나", Integer.toString(Max_date_of_month));


        //약속시간: 현재시간+ <1시간 반> :::::::::내가 정한거(왜나면 붕어방-서울역 1시간 거리

        //분
        if(which_minute>=30){ //1시간 반 더하면: 2시간 올라가고 30분이 줄어든다.
            which_minute=which_minute-30;//2시간 더해줘야해
            //시
            if(which_hour==22){
                which_hour=0;//하루 늘려줘야해
                //일
                if(which_date==Max_date_of_month){
                    which_date=1;//한달 늘려줘야해
                    //달
                    if(which_month==12){
                        which_month=1;//1년 늘려줘야해
                        //년
                        which_year=which_year+1;
                    }
                    else if(which_month!=12){
                        which_month=which_month+1;
                    }
                }
                else if(which_date<Max_date_of_month){
                    which_date=which_date+1;
                }
            }
            else if(which_hour==23){
                which_hour=1;//하루 늘려줘야해
                if(which_date==Max_date_of_month){ //달의 마지막 날인 경우
                    which_date=1;//한달 늘려줘야해
                    if(which_month==12){//12월인 경우
                        which_month=1;//1년 늘려줘야해
                        which_year=which_year+1;
                    }
                    else if(which_month!=12){
            ...
            else if(which_hour<22){
                which_hour=which_hour+2;
            }
        }
        //분
        else if(which_minute<30){
            which_minute=which_minute+30;//1시간 더해줘야해
            //시
            if(which_hour==23){
                which_hour=0; //하루 더해줘야해
                //일
                if(which_date==Max_date_of_month){
                    which_date=1;//한달 올려줘야해
                    if(which_month==12){
                        which_month=1;
                        which_year=which_year+1;
                    }
        ...
        String appointment_year = String.valueOf(which_year); //4자리
        String appointment_month = String.valueOf(which_month); //1,2자리
        String appointment_date = String.valueOf(which_date); //1,2자리
        String appointment_hour = String.valueOf(which_hour); //1,2자리
        String appointment_minute = String.valueOf(which_minute); //1,2자리

        //2자리 수로 맞추는 과정
        if(appointment_month.length()==1){
            appointment_month = "0"+appointment_month;
        }
        if(appointment_date.length()==1){
            appointment_date = "0"+appointment_date;
        }
        if(appointment_hour.length()==1){
            appointment_hour = "0"+appointment_hour;
        }
        if(appointment_minute.length()==1){
            appointment_minute = "0"+appointment_minute;
        }

        //현재시간+1시간 반 더한 값:today_time_appointment
        String today_time_appointment =appointment_year+appointment_month+appointment_date+appointment_hour+appointment_minute+"00";
Log.d("약속 도착 시간:현시각+1시간반",today_time_appointment);

        //원래는 스케쥴의 시작 시간 받아와.
        str_time_appointment=today_time_appointment;           //임의로 설정.
        //원래는 스케쥴의 장소(위도,경도) 받아와.
            ...
            handler.post(new Runnable() { @Override public void run() {
                ...
                //값이 정상일 때, 본격적 서비스.
                    ...
                    //출발 15분 전인 경우.
                        ...
                        //위에 거 대신 오른쪽 setVibrate 써도 돼:noti 진동시키는법 =>> //mBuilder.setVibrate(vibrate);
